import { Injectable, Injector, Renderer2, RendererFactory2, ɵsetClassDebugInfo } from '@angular/core';
...
export class FlowchartConnectorsService {
  ...
  private drawPath(
    ...
    const hypotenuse = Math.min(Math.sqrt(deltaX ** 2 + deltaY ** 2) / 3, 30);
    ...
    if (animate) {
      const pathLength = Math.sqrt((parentElXCenter - childElXCenter) ** 2 + (childElYTop - parentElYBottom) ** 2);
